...
The retrieval pipeline now uses four stages:

1. A bounded helper call creates an optional HyDE passage and any stepback/CoT
   retrieval questions together. HyDE is embedded for dense search only; BM25
   continues to use the real user or planner query.
2. Dense embedding search and BM25, fused with reciprocal-rank fusion (RRF),
   produce a broad candidate set.
3. A selected evidence reranker scores whether each candidate contains evidence
...
4. Only passages that meet a calibrated evidence policy are sent to the answer
   model. The selected context is diversified across papers.

The hypothetical HyDE passage is never included in answer context, citations,
or validation input. It is retrieval scaffolding, not medical evidence.
...
| `RAG_HYDE_ENABLED` | `false` | Generate a hypothetical biomedical passage for the primary dense embedding. Production enables this explicitly. |
| `HYDE_MAX_TOKENS` | `320` | Combined completion budget for the HyDE passage and any bounded agent retrieval questions. |

Set `HELPER_CHAT_MODEL` to a lower-latency model that supports structured JSON
output if the answer model is expensive. HyDE and agent planning share one
structured helper call, keeping the production request ceiling at four external
operations. The LLM reranker makes one batched
...
```bash
python generate_dialysiscare_responses.py \
  --force \
  --output /tmp/pipeline_results_with_dialysiscare_fresh.json

python evaluate_retrieval.py \
  --benchmark app/pipeline_results.json \
  --predictions /tmp/pipeline_results_with_dialysiscare_fresh.json \
  --k 1,3,5,10 \
  --label-level paper \
  --per-query \
...
`generate_dialysiscare_responses.py` pins standard RAG (rather than adaptive,
...
`DIALYSISCARE_BACKEND_URL` to the environment that actually contains the rebuilt
