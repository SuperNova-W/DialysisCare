#!/usr/bin/env python3
"""
Augment pipeline_results.json with the live DialysisCare RAG bot's answer + metadata.

For every question in the input JSON this script calls the configured DialysisCare
...
to each record:
  ...
  - "dialysiscare_response": the bot's answer text
  - "dialysiscare_metadata": the sources/citations + validation + follow-ups the bot used

The original file is left untouched; results are written to a copy.

Usage:
    python3 generate_dialysiscare_responses.py \
        [--input  backend/app/pipeline_results.json] \
        [--output backend/app/pipeline_results_with_dialysiscare.json] \
...
DEFAULT_OUTPUT = os.path.join(HERE, "app", "pipeline_results_with_dialysiscare.json")
...
def main() -> int:
    ...
    if os.path.exists(args.output) and not args.force:
        try:
            ...
            for rec in records:
                ...
                if prev and prev.get("dialysiscare_response") is not None:
                    rec["dialysiscare_response"] = prev["dialysiscare_response"]
                    rec["dialysiscare_metadata"] = prev.get("dialysiscare_metadata")
    ...
    todo = [
        ...
        if rec.get("dialysiscare_response") is None
    ]
    if args.limit:
    ...
    def worker(idx: int):
        ...
        try:
            ...
            rec["dialysiscare_response"] = data.get("response", "")
            rec["dialysiscare_metadata"] = build_metadata(data)
            status = "ok"
        except Exception as e:  # noqa: BLE001
            rec["dialysiscare_response"] = None
            rec["dialysiscare_metadata"] = {"error": str(e)}
            status = "err"
        with lock:
            done["n"] += 1
            done[status] += 1
