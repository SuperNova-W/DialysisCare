...
ANSWER_SYSTEM_PROMPT = """You are DialysisCare, a warm and practical medical-information assistant specializing in peritoneal dialysis and kidney disease.

The user may ask a short conversational follow-up without repeating "PD" or
"peritoneal dialysis." Infer that context from the supplied peritoneal dialysis
literature instead of rejecting the question merely because it lacks a domain
keyword.
...
a conversational follow-up, briefly explain that DialysisCare focuses on
peritoneal dialysis and kidney disease. Never force an unrelated question into
...
async def retrieve(
    ...
    if mode != "standard_rag" or hyde_enabled:
        try:
            plan = await service.plan_retrieval(query, mode, tracker)
            planned_queries = [
                str(item).strip()
            ...
            if hyde_enabled:
                hypothetical_passage = str(
                    plan.get("hypothetical_passage") or ""
                ).strip()
        except ProviderUnavailable as error:
            logger.warning(
                "Bounded HyDE/planner unavailable; falling back to original query (%s)",
                error.code,
            )
    search_queries = [search_query]
    embedding_inputs = [hypothetical_passage or search_query]
    ...
    for planned in planned_queries:
        ...
        if expanded.lower() not in seen_queries:
            ...
            embedding_inputs.append(expanded)
            seen_queries.add(expanded.lower())
    query_embeddings = await service.embeddings(embedding_inputs, tracker)
    ...
    metadata = {
        **metadata,
        ...
        "hyde_used": bool(hypothetical_passage),
...
def fallback_followup_questions(query: str) -> List[str]:
    ...
    lowered = query.lower()
    if any(term in lowered for term in ("treat", "medication", "icodextrin")):
        return [
    ...
    if any(term in lowered for term in ("symptom", "pain", "sign")):
        return [
            ...
            "How are these symptoms usually evaluated in people on peritoneal dialysis?",
            "What can I track before my next nephrology appointment?",
        ]
    return [
        "How is this usually monitored over time in people on peritoneal dialysis?",
